...
import { Array, Effect, HashMap, Layer, Match, Schema, Stream, SubscriptionRef, pipe } from "effect"
...
class TodoService extends Effect.Service<TodoService>()("TodosService", {
    ...
    effect: Effect.gen(function* () {
        ...
        const adTodosRef = yield* SubscriptionRef.make<
            AsyncData.AsyncData<
                ReadonlyArray<Todo>,
                HttpApiDecodeError | HttpClientError | ParseError | InternalApplicationError | TodoNotFound
            >
        >(AsyncData.noData())

        const todosFilter = yield* SubscriptionRef.make<TodosFilter>("all")

        const filteredTodos = Stream.zipLatestAll(adTodosRef.changes, todosFilter.changes).pipe(
            Stream.map(([adTodos, filter]) =>
                AsyncData.map(adTodos, (todos) =>
        ...
        const incompleteTodosCount = adTodosRef.changes.pipe(
            Stream.map((adTodos) =>
                pipe(
                    adTodos,
                    AsyncData.map((todos) => todos.length),
        ...
        const getTodo = (todos: ReadonlyArray<Todo>, id: TodoId) =>
            Array.findFirst(todos, (t) => t.id === id).pipe(
                Effect.catchTag(
